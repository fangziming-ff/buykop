...
@SysThread(display = "保存子系统的配置文件", sys = Constants.current_sys, interval = 10, execCycle = "")
public class AutoSaveSysConfigJson implements TreadInf{
	
	
	private static Logger  logger=LoggerFactory.getLogger(AutoSaveSysConfigJson.class);
			...
			try {
				...
				try {
					...
					if(true) {
						...
						root.setStatus(1L);
						QueryListInfo<PRoot> list=service.getMgClient().getList(root,service);
						for(PRoot x:list.getList()) {
							try {
								ConfigUtil.saveToJson(x.getCode(),service);
							}catch(Exception e) {
								e.printStackTrace();
							}
						}
